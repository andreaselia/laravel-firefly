...
<x-card>
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-2">
            @if (config('firefly.features.correct_post'))
                @if($post->is_correct)

                @else
                @can ('mark', $post)
                    <a href="{{ route('firefly.post.correct', $post) }}">
                        <span class="sr-only">{{ __('Mark as Correct') }}</span>
                        <svg class="w-5 h-5 text-gray-700 hover:text-gray-500" data-tippy-content="{{ __('Mark as Correct') }}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
